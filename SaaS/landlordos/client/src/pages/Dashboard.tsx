...
const Dashboard: React.FC = () => {  const { user } = useAuth();
  ...
  // Property performance pie chart data
  const propertyTypeData = overview?.performanceData?.map((property, index) => ({
    name: property.address.substring(0, 20) + '...',
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100">
      ...
      <div className="relative mb-8">
        ...
        <GlassMorphismCard className="p-8 m-0">
                ...
                <div className="bg-white/15 backdrop-blur-sm rounded-2xl p-6 border border-white/20">
                  <div className="text-center">
                    ...
                    <div className={`text-2xl font-bold ${(stats?.monthlyNetIncome || 0) >= 0 ? 'text-green-300' : 'text-red-300'}`}>
                      ${stats?.monthlyNetIncome?.toLocaleString() || 0}                    </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-8 mb-8">
        {/* Financial Trends Chart with Enhanced Styling */}
        ...
        {/* Property Performance with Progress Ring */}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
        {/* Monthly Comparison with Enhanced UI */}
                    ...
                    <motion.div 
                      ...
                      initial={{ width: 0 }}                      animate={{ 
                        width: `${(stats?.monthlyRentIncome && stats?.monthlyExpenses) 
                          ? Math.min(((stats?.monthlyRentIncome || 0) / ((stats?.monthlyRentIncome || 0) + (stats?.monthlyExpenses || 0))) * 100, 100)
                          : 0}%`
                      }}
                ...
                <div>
                  ...
                  <div className="w-full bg-gray-200 rounded-full h-3">
                    <motion.div 
                      ...
                      initial={{ width: 0 }}                      animate={{ 
                        width: `${(stats?.monthlyRentIncome && stats?.monthlyExpenses) 
                          ? Math.min(((stats?.monthlyExpenses || 0) / ((stats?.monthlyRentIncome || 0) + (stats?.monthlyExpenses || 0))) * 100, 100)
                          : 0}%`
                      }}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
        ...
        <div className="xl:col-span-2 bg-white overflow-hidden shadow-lg rounded-2xl border border-gray-100">
          <div className="p-8">
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center">
                ...
                <h3 className="ml-3 text-xl font-semibold text-gray-900">Financial Trends</h3>
              </div>
        ...
        <div className="bg-white overflow-hidden shadow-lg rounded-2xl border border-gray-100">
          <div className="p-8">
            <div className="flex items-center mb-6">
              ...
              <h3 className="ml-3 text-xl font-semibold text-gray-900">Property Performance</h3>
            </div>
      ...
      {/* Lower Section - Activity and Insights */}
        ...
        <div className="bg-white overflow-hidden shadow-lg rounded-2xl border border-gray-100">
          <div className="p-6">
            <div className="flex items-center mb-6">
              ...
              <h3 className="ml-3 text-lg font-semibold text-gray-900">Monthly Overview</h3>
            </div>
            <div className="space-y-6">
              <div>
                <div className="flex justify-between items-center mb-3">
                ...
                <div className="w-full bg-gray-200 rounded-full h-3">
                  <div 
                    className="bg-gradient-to-r from-green-400 to-green-600 h-3 rounded-full transition-all duration-500"                    style={{
                      width: `${(stats?.monthlyRentIncome && stats?.monthlyExpenses) 
                        ? Math.min(((stats?.monthlyRentIncome || 0) / ((stats?.monthlyRentIncome || 0) + (stats?.monthlyExpenses || 0))) * 100, 100)
                        : 0}%`
                    }}
        ...
        <div className="bg-white overflow-hidden shadow-lg rounded-2xl border border-gray-100">
          <div className="p-6">
            <div className="flex items-center mb-6">
              ...
              <h3 className="ml-3 text-lg font-semibold text-gray-900">Upcoming Tasks</h3>
            </div>
            <div className="space-y-3">
              {overview?.upcomingReminders?.slice(0, 4).map((reminder) => (
                <div key={reminder.id} className="p-4 bg-gradient-to-r from-yellow-50 to-orange-50 rounded-xl border-l-4 border-orange-400">
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <p className="text-sm font-medium text-gray-900">{reminder.title}</p>
                      <p className="text-xs text-gray-500 mt-1">
        ...
        <div className="p-8">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center">
              ...
              <h3 className="ml-3 text-xl font-semibold text-gray-900">Active Leases</h3>
            </div>
            <span className="text-sm text-gray-500">
              {overview?.activeLeases?.length || 0} active leases
            </span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {overview?.activeLeases?.slice(0, 6).map((lease) => (
              <div key={lease.id} className="p-4 bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl border border-blue-100 hover:shadow-md transition-all duration-200">
                <div className="flex items-start justify-between mb-3">
                  <div className="flex-1">
                    <p className="text-sm font-semibold text-gray-900">{lease.tenantName}</p>
                    <p className="text-xs text-gray-600 mt-1">{lease.propertyAddress}</p>
                  </div>
                  <span className="text-lg font-bold text-blue-600">
                    ${lease.monthlyRent.toLocaleString()}
